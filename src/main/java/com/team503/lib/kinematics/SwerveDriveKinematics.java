...
public class SwerveDriveKinematics {
   ...
   *               of the robot.
   */
  public SwerveDriveKinematics(Translation2d... wheels) {
    if (wheels.length < 2) {
      throw new IllegalArgumentException("A swerve drive requires at least two modules");
    }
    m_numModules = wheels.length;
    m_modules = Arrays.copyOf(wheels, m_numModules);
    ...
    if (centerOfRotation.getX() != m_prevCoR.getX()
      ...
      for (int i = 0; i < m_numModules; i++) {
        m_inverseKinematics.setRow(i * 2 + 0, 0, /* Start Data */ 1, 0,
            -m_modules[i].getY() + centerOfRotation.getY());
        m_inverseKinematics.setRow(i * 2 + 1, 0, /* Start Data */ 0, 1,
            +m_modules[i].getX() - centerOfRotation.getX());
      }
  ...
  public static void normalizeWheelSpeeds(SwerveModuleState[] moduleStates,
    ...
    double realMaxSpeed = Collections.max(Arrays.asList(moduleStates)).speed;
