 ...
 */
public class AndyArm extends Subsystem implements SuperStructureSystem {
  ...
  private static AndyArm instance = new AndyArm();

  public static AndyArm getInstance() {
  ...
  public synchronized void updateSuperstruture() {
    if ((DriverStation.getInstance().isAutonomous() || !RobotState.getInstance().getIsManual())) {
      ...
      if (Robot.bot.hasWrist()) {
        ...
        if (wristPower > Robot.bot.MAX_WRIST_POWER) {
          ...
          if (!(RobotState.getInstance().getSuperStructurePreset() == SuperStructurePreset.FRONT_CARGO_BUS
              && AndyArm.getInstance().getEncoderDeg() < -30)) {
      ...
      if (Robot.bot.hasArm()) {
        ...
        if (AndyArm.getInstance().getEncoderDeg() > -47) {
      ...
      if (Robot.bot.hasArm()) {
        ...
        if (AndyArm.getInstance().getEncoderDeg() > 0.0 && AndyArm.getInstance().getEncoderDeg() < 180.0) {
          eLim = Robot.bot.gArmExtLength * (1 / Math.cos(Math.toRadians(AndyArm.getInstance().getEncoderDeg())) - 1);
          eLim = Math.abs(eLim);
        } else {
          eLim = 0.0;
        }
